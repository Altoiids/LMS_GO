...
func ProfileBooks(username string) ([]types.Book, error) {
	...
	rows, err := db.Query("SELECT b.book_id, b.book_name, b.publisher FROM request r JOIN books b ON r.book_id = b.book_id WHERE r.user_id =? and r.status = 'owned'",userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
